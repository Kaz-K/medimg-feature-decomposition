# Credit for the original script goes to moskomule, which can be found at https://github.com/moskomule.
...
def _faiss_knn(keys: torch.Tensor,
    ...
    scores = keys.new_zeros(
        (queries.size(0), num_neighbors), dtype=torch.float32)
    indices = keys.new_zeros(
        (queries.size(0), num_neighbors), dtype=torch.int64)
...
class VQModule(nn.Module):
    ...
    def _quantize(self,
        ...
        dist, ids = k_nearest_neighbor(
            self.embed, flatten, 1, 'l2', backend=self._knn_backend)
        # embed_onthot: (BHW)x{dict_size}
        embed_onehot = F.one_hot(
            ids.view(-1), self.dict_size).to(flatten.dtype)
        ...
        if self.training:
            ...
            exponential_moving_average_(
                self.cluster_size, embed_onehot_sum, self.momentum)
            exponential_moving_average_(
                self.embed_avg, embed_sum, self.momentum)

            n = self.cluster_size.sum()
            cluster_size = n * (self.cluster_size + self.eps) / \
                (n + self.dict_size * self.eps)
